...
try {
  ...
  writeDoc(p.id, '正文/第01章_雾港.md', FM(1, '雾港') + '陈师傅推门。陈默抬头。老陈坐下。陈默开口。陈师傅打断了他。\n')
  ...
  assert('共 2 条（陈默/沈藏 各一章）', res && res.items.length === 2)

  const first = res.items.find((i) => i.target === '人物/陈默.md')
  assert('条目 low/character/target=陈默档', first && first.severity === 'low' && first.type === 'character' && first.target === '人物/陈默.md')
  assert('what 列称呼/处数/交替次数/「若并非刻意」', first && first.what.includes('「陈师傅」「陈默」「老陈」') && first.what.includes('交替 4 次') && first.what.includes('若并非刻意'))
  ...
  // ② 反例：同姓双雄（陈默+陈航，生成称谓「陈师傅/老陈」归属不明→全局弃用）→ 陈默只剩全名 → 不报（第01/03章）
  writeDoc(p.id, '人物/陈航.md', PERSON('陈航'))
  writeDoc(p.id, '正文/第03章_码头.md', FM(3, '码头') + '陈师傅推门。陈默抬头。陈师傅坐下。陈默开口。陈师傅打断了他。\n')
  ...
  assert('第01章陈默同被全局归属污染 → 不报（口径：变体全局唯一归属）', r2.ok === true && (r2.ok ? r2.result.items.filter((i) => i.target === '人物/陈默.md') : []).length === 0)
