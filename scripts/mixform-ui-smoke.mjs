...
try {
  ...
  const btn = await page.eval(`(() => {
    const b = [...document.querySelectorAll('[role="menuitem"]')].find((x) => (x.title || '').startsWith('称谓混用核查'))
    return b ? { title: b.title } : null
  })()`)
  check('菜单项存在（title=「称谓混用核查…」）', !!btn)

  // ② 点击菜单项 → 抽屉打开、标题与本地规则文案出现
  console.log('点击菜单项:', await page.eval(clickByTitle('称谓混用核查（本地规则·秒级）')))
