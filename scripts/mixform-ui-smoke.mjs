...
try {
  ...
  const btn = await page.eval(`(() => {
    const b = [...document.querySelectorAll('button')].find((x) => x.title && x.title.startsWith('称谓混用核查'))
    return b ? { title: b.title, label: b.getAttribute('aria-label') } : null
  })()`)
  check('新按钮存在（title + aria-label）', btn && btn.label === '称谓混用核查')

  // ② 点击 → 抽屉打开、标题与本地规则文案出现
  console.log('点击按钮:', await page.eval(clickByTitle('称谓混用核查：同章叙述层交替用多个称呼指同一人（本地规则·秒级·零模型）')))
  await evalUntil(page, `document.body.innerText.includes('称谓混用核查')`, (v) => v === true, 8000, '抽屉标题出现')
