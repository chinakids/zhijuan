...
const DATA_LEXICON = ['单线回归章', '测试类别', '帮我再改一段', '我的项目', '需要一段描写', '模板冒烟项目', '（修订）', '# 标题字面'] // （修订）=skill-settings-ui-smoke D3 编辑回显的输入值尾缀（2026-09-22 人工核：setField 写入描述「…（修订）」后沿行回显断言，非徽标断链）；# 标题字面=paste-plain-ui-smoke 粘贴输入的测试文本（2026-09-23 人工核：模拟剪贴板字面内容，非 UI 字面量）
