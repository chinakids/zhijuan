// 首页作品编译 v1.1（Word 导出）· UI 冒烟（无头 Chrome CDP 9224 + devShim）
// 场景：首页卡片「更多操作」→「导出作品（Word）」→ devShim compileExportDocx 返回 ok+path+chapters
//       → toast「已导出 Word 作品」（含 _成品.docx 路径）出现；全程零 JS 异常。
...
// [3] 点击 Word 项 → devShim 返回 ok+path+chapters → toast「已导出 Word 作品」出现
const clicked = await evalJs(`(() => {
  const items = [...document.querySelectorAll('[role="menuitem"], button')]
  const b = items.find((x) => x.innerText.includes('导出作品（Word）'))
  if (!b) return 'no-item'
  b.click()
  return 'ok'
})()`)
check('点击导出作品（Word）项', clicked === 'ok', String(clicked))

let toastShown = ''
for (let i = 0; i < 20; i++) {
  toastShown = await evalJs(`[...document.querySelectorAll('[role="status"]')].map((x) => x.innerText).join('|')`)
  if (toastShown.includes('已导出 Word 作品')) break
  await sleep(500)
}
check('成功 toast「已导出 Word 作品」出现（含 _成品.docx 路径）', toastShown.includes('已导出 Word 作品') && toastShown.includes('_成品.docx'), toastShown)
