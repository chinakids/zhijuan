...
try {
  ...
  // ① Agent 面板有「人物在场与称谓核查」入口
  const hasEntry = await page.eval(`[...document.querySelectorAll('button')].some((b) => (b.title || '').includes('人物在场与称谓核查'))`)
  if (!hasEntry) throw new Error('未找到「人物在场与称谓核查」入口按钮')
  console.log('OK 「人物在场与称谓核查」入口按钮存在')

  // ② 点击入口 → 抽屉出现（标题 = 人物在场与称谓核查）
  console.log('点击入口:', await page.eval(clickByTitle('人物在场与称谓核查：约定头「涉及人物」vs 正文本名/登记别名（本地规则·秒级·零模型）')))
