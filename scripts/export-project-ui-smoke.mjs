// 首页项目导出·UI 冒烟（无头 Chrome CDP 9224 + devShim）
...
const itemShown = await evalJs(`(() => {
  ...
  return items.some((x) => x.innerText.includes('导出到…'))
})()`)
check('菜单含「导出到…」', itemShown === true)

// [3] 点击「导出到…」→ devShim 返回 ok+dest → toast 出现
const clicked = await evalJs(`(() => {
  const items = [...document.querySelectorAll('[role="menuitem"], button')]
  const b = items.find((x) => x.innerText.includes('导出到…'))
  if (!b) return 'no-item'
  b.click()
  return 'ok'
})()`)
check('点击导出到…', clicked === 'ok', String(clicked))
...
for (let i = 0; i < 20; i++) {
  ...
  if (toastShown.includes('已导出')) break
  await sleep(500)
}
check('成功 toast「已导出」出现（含目标路径）', toastShown.includes('已导出') && toastShown.includes('demo-aseya'), toastShown)
