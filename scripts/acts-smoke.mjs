...
writeFileSync(
  ...
  [
    ...
    "    const pass = r.acts >= 1 && words > 200 && hasFm && hasPrevTail",
    "    if (!pass) console.log('[DIAG] 分幕空/弱：failed=' + JSON.stringify(r.failed ?? []) + '；失败段原始回复（各前 800 字）:\\n' + (Object.keys(r.failedRaw ?? {}).length ? Object.entries(r.failedRaw).map(([k, v]) => '第' + k + '段: ' + v.slice(0, 800)).join('\\n') : '<无 failedRaw>'))",
    "    exitCode = pass ? 0 : 3",
    '  } else exitCode = 1',
    '} catch (e) {',
