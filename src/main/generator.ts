...
/** 把整个项目结构化为给 LLM 的输入文本 */
function buildPrompt(project: Project, chapter: Chapter): string {
  const chars = project.characters
    .map((c, i) => {
      const fields = c.fields.map((f) => `      - ${f.key}: ${f.value}`).join('\n')
      return `【人物${i + 1}】
  名字: ${c.name}（${c.role}，${c.age}岁${c.isProtagonist ? '，主角' : ''}）
  标签: ${c.tags.join('、')}
  身世: ${c.background}
  与主角关系: ${c.relation}
  身体档案:
${fields}`
  ...
  const curves = chapter.curves.map((c) => {
  ...
  return `你是网文作家，正在写一部都市小说。以下是本项目的世界观、人物设定、本章的要素与情节曲线。
...
async function generate(
  ...
  const prompt = buildPrompt(project, chapter)
  _abort?.abort()
