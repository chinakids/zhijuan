...
5. **skill 运行层上下文注入面**（~~2026-09-21 09:00 智能层轮登记~~；**2026-09-21 12:00 已实现收口**，基线=docs/skill-运行层-产品规划-2026-09-21.md，主责=智能层）——runChat system 段新增【可用技能】清单块（description 常驻；预算 SKILL_CAPS 新常量入 shared/contextCaps.ts：每技能 ≤120 字符+清单总 ≤600 字符，超预算 name-only）+ 激活【技能：<名>】正文块（≤3000 字符，超限保头+注明 zj_read_doc 现读）；匹配=主进程 matchSkillForInput 纯函数（triggers→description，≤2 条，disabled 不参与；不做 embedding=二期候选）；子任务/检查类/runSync 不注入（设计决定）；验收=单测 31 例+skill-live.mjs 探针（A/C PASS，B 显式路径三次超时=引擎 dsh-tool-skill 干扰，登记治本候选）+skill-menu-ui-smoke 5/5+context-full-live 零回归（本轮未改 context.ts 装配）；**遗留观察**：引擎自带 skill 工具（dsh-tool-skill）与织卷技能目录未对接→显式路径真模型行为不确定（候选 1）；**（2026-09-22 补充：增量 4「定时学习写作习惯」按设计文档 §6 交叉面=草稿生成后注入走既有注入面无新逻辑，已登记 01-智能层.md 候选 1 推进；增量 4a 纯函数层 05ff704 已落地，不动本线实现）**。
...
- 落地：`src/shared/wordfreq.ts`＝`overuseCheck/overuseItems/countPhraseInText/visibleBodyOf/BUILTIN_OVERUSE`（59 条中文口头禅/AI 腔词表，opts.dict 可扩展）——词表式频率报告（count/perK/章分布/severity≥50 high/≥15 med/≥3 low 实据注释）；15 单测绿 + 真数据命中 18 条（high 7 全书级口头禅/零虚词误报）；三道门 970 例绿；无 UI 变化（纯函数轮不言）。
