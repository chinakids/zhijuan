...
>
> 2026-09-17 18:00 智能层轮更新：候选 1（预算改动后长效探针复测）**收口**——context-full-live（26s）+ mixform-toagent-live（6/6，285s）全过，预算改动注入链**零漂移实证**；「预算/装配改动后复跑两探针」固化为固定后置动作（换模型后亦复跑）。明细见智能层档案 18:00 轮。
