...
describe('buildWritingContext（写作上下文装配）', () => {
  it('完整装配：当前章、上一章尾部、最多前4位人物、切片、章卡、素材索引；且约定头被剥掉', async () => {
    ...
    expect(mat).toContain('素材库/桥段/旧物定情（标签：桥段、旧物、相遇）：用一个旧物件串起两人第一次真正交集的场景')
    expect(mat).toContain('素材库/环境/雾海夜航（标签：环境、雾、夜）：大雾的夜里，港口的能见度往往不足五十米')
...
describe('预算截断可见性（2026-09-13 上下文审计第二轮收口）', () => {
  ...
  it('素材库路标未超预算：素材文件（类别/标题/标签/首段预览）原样全量装配，零提示；索引.md/采集池不注入', async () => {
    ...
    expect(all).toContain('- 素材库/桥段/旧物定情（标签：桥段、旧物、相遇）：用一个旧物件串起两人第一次真正交集的场景')
